    ...
    for side in ['ORANGE', 'PURPLE', 'GREEN', 'BLUE']:
        if side in data['initial_deployments']:
            ...
            for item in data['initial_deployments'][side]:
                ...
                b.place(player, pid, (r,c))
    print('部署完成。'); print(ascii_board(b, Player.ORANGE, reveal_all=True))
    save_four_player_views(b, out_dir=args.renders, stem='replay')
    t=1
    for mv in data['moves']:
        if args.step: input('回车下一手...')
        time.sleep(args.sleep)
        player = Player[mv['player']]  # 支持四个玩家
        ...
        print(ascii_board(b, Player.ORANGE, reveal_all=True))
        save_four_player_views(b, out_dir=args.renders, stem=f'replay')
        t+=1
    print('复盘结束。')
