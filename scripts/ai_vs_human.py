...
def main():
    ...
    ap.add_argument('--human_side', choices=['ORANGE','PURPLE','GREEN','BLUE'], default='ORANGE')
    ...
    human = getattr(Player, args.human_side)
    ...
    logger = ReplayLogger()
    logger.set_players('HUMAN' if p == human else 'AI' for p in PLAYER_ORDER)
    ...
    while game.state.phase == 'deploy':
        ...
        if cur == human and args.human_random_deploy:
            ...
            while game.state.phase == 'deploy' and game.state.turn == human:
                pid, rc, _ = agents[human].select_deploy(game, human)
            ...
            print('你的回合-部署。输入: PIECE_NAME r,c  例如: JUNQI 16,7')
            ...
            if game.deploy(cur, pid, rc): 
                logger.log_deploy(cur, pid, rc)
            else: 
                print('非法位置或该棋子已用完')
        else:
            pid, rc, _ = agents[cur].select_deploy(game, cur)
            game.deploy(cur, pid, rc)
            logger.log_deploy(cur, pid, rc)
            print(f'AI {cur.name} 部署: {pid.name} @ {rc}')
    ...
    while not game.is_over():
        ...
        if game.state.turn == human:
            ...
            if not ev.get('ok'): 
                ...
                logger.log_move(turn_idx, human, src, dst, ev)
                turn_idx += 1
        else:
            player = game.state.turn
            src, dst, _, _ = agents[player].select_move(game, player)
            ev = game.step(src, dst)
            logger.log_move(turn_idx, player, src, dst, ev)
            turn_idx += 1
            print(f'AI {player.name}: {src}->{dst}  事件={ev}')
